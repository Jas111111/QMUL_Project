...
def magnitude_pruning(model, amount=0.3, layers_to_skip=None):
    """
...
def prune_model(model, train_loader, val_loader, device, method='magnitude', 
               amount=0.3, fine_tune=True, epochs=5, lr=0.001):
    """
    ...
    """
    if method == 'magnitude':
        pruned_model = magnitude_pruning(model, amount=amount)
    elif method == 'structured':
        pruned_model = structured_pruning(model, amount=amount)
